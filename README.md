...
Forecast daily sales with ARIMA/SARIMA on synthetic data. Includes generation, cleaning, train/validation split, AIC-based model search, evaluation (RMSE/MAPE), and 90-day forecast with confidence intervals. Tested, reproducible scripts and charts for portfolio showcase.
...
- Unit tests + CI quality gate (ruff, black, mypy, pytest)
...
│  └─ daily_sales.csv        # sample data (regenerate with the script)
├─ src/
│  ├─ forecast_arima.py
│  └─ metrics.py
├─ tests/
...
└─ outputs/                  # generated by src/forecast_arima.py (git-ignored)
```
...
## Development
Install the dev toolchain and run the same checks CI runs:
```bash
pip install -r requirements-dev.txt

ruff check .          # lint (E, F, I, B, SIM, UP)
black --check .       # formatting
mypy src data         # type checking
